...
# Set S3 configuration for Hadoop (not used in pyspark-airflow but keeping for consistency)
export HADOOP_OPTS="-Dfs.s3a.access.key=admin -Dfs.s3a.secret.key=admin -Dfs.s3a.endpoint=http://minio:9000 -Dfs.s3a.path.style.access=true"
